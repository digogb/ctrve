...
If any are missing, inform the user that requirement artifacts are a prerequisite (generated by `tjce-agent-requirements`) and stop. Never invent or assume requirements.
...
If the user's intent is ambiguous, present both options. If `--headless build` or `--headless verify`, route directly.
