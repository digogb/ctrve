---
name: "tjce-setup"
description: Instala e configura o modulo TJCE no projeto. Use when the user requests to 'instalar modulo tjce', 'configurar tjce', 'setup tjce', or 'install tjce module'.
---
...
3. Check for per-module configuration at `{project-root}/_bmad/tjce/config.yaml` and `{project-root}/_bmad/core/config.yaml`. If either file exists:
...
python3 ./scripts/merge-help-csv.py --target "{project-root}/_bmad/module-help.csv" --source ./assets/module-help.csv --legacy-dir "{project-root}/_bmad" --module-code tjce
```
...
## Scaffold TJCE Project Structure

After creating output directories, scaffold the TJCE-specific subdirectories:

```bash
mkdir -p {resolved_output_folder}/requirements
mkdir -p {resolved_output_folder}/architecture
mkdir -p {resolved_output_folder}/tests
mkdir -p {resolved_output_folder}/ux
mkdir -p {resolved_output_folder}/reports
mkdir -p {resolved_output_folder}/release/apf
mkdir -p {resolved_output_folder}/release/manual
```

Then check if `CLAUDE.md` exists at project root. If not, generate a template:
...
Este projeto segue a Esteira de Desenvolvimento AI-Native do TJCE.

## Artefatos Obrigatorios (PDS Unificado)

- Estorias de Usuario (requirements/user-stories.md)
- Regras de Negocio (requirements/business-rules.md)
- Mensagens do Sistema (requirements/messages.md)
- Visao do Produto (requirements/product-vision.md)
- Casos de Teste (tests/test-cases.md)
- Arquitetura Tecnica (architecture/tech-design.md)

## Classificacao de Erros TJCE

| Severidade | Descricao | SLA |
| ---------- | --------- | --- |
| Critico | Sistema indisponivel ou perda de dados | 4h |
| Alto | Funcionalidade principal comprometida | 24h |
| Medio | Funcionalidade secundaria com workaround | 72h |
| Baixo | Cosmetico ou melhoria | Proximo sprint |

## Seguranca

- Dados judiciais sao sigilosos por padrao
- Nunca expor CPF, numero de processo ou dados pessoais em logs
- Toda comunicacao com APIs externas deve usar HTTPS
- Credenciais e tokens nunca em codigo fonte

## Esteira de Desenvolvimento

SPEC -> GATE CHECK -> BUILD -> VERIFY -> SHIP

Use 'bmad-help' para ver os skills disponiveis e sua sequencia.
```

If `CLAUDE.md` already exists, do not overwrite — inform the user the template was skipped.
...
```bash
python3 ./scripts/cleanup-legacy.py --bmad-dir "{project-root}/_bmad" --module-code tjce --also-remove _config --skills-dir "{project-root}/.claude/skills"
```
